...
mod tests {
    ...
    #[test]
    fn apply_thresholding_disabled_returns_input_unchanged() {
        let data = vec![1.0, 2.0, 3.0];
        let result = apply_thresholding(&data, 2.0, false);
        assert_eq!(result, data);
    }

    #[test]
    fn apply_thresholding_filters_within_sigma() {
        // Mostly-zero data with a couple of large outliers.
        let mut data = vec![0.0; 100];
        data[0] = 100.0;
        data[1] = -100.0;
        let result = apply_thresholding(&data, 1.0, true);
        assert!(result.len() < data.len());
        assert!(result.contains(&100.0) || result.contains(&-100.0));
    }

    #[test]
    fn build_histogram_counts_sum_to_input_length_when_in_range() {
        let data: Vec<f64> = (0..1000).map(|i| i as f64).collect();
        let (counts, bin_centers) = build_histogram(&data, 0.0, 1000.0, 100, false);
        let total: f64 = counts.iter().sum();
        assert_eq!(total, 1000.0);
        assert_eq!(bin_centers.len(), 100);
    }

    #[test]
    fn calculate_mean_parallel_matches_manual_average() {
        let mut a = BaselineData::default();
        a.l1[0] = 10.0;
        let mut b = BaselineData::default();
        b.l1[0] = 20.0;
        let data = vec![a, b];
        let means = calculate_mean_parallel(&data, |d| d.l1);
